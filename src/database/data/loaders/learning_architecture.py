...
def load_learning_architecture(app, file_path):
    ...
    with app.app_context():
        ...
        for stream in streams:
            ...
            stream_slug_array = stream['stream'].split(' ')
            if len(stream_slug_array) == 1:
                new_stream.slug = stream_slug_array[0].lower()
            else:
                stream_slug_array = list(filter(lambda x: x is not None, 
                    list(map(lambda x: x.lower() if not x in punct else None, stream_slug_array))))
            ...
            for practice in stream_practices:
                query = learning_practices_cache.get(practice)
                if query is None:
                    ...
                    practice_slug_array = practice.split(' ')
                    if len(practice_slug_array) == 1:
                        new_learning_practice.slug = practice_slug_array[0].lower()
                    else:
                        practice_slug_array = list(filter(lambda x: x is not None, 
                            list(map(lambda x: x.lower() if not x in punct else None, practice_slug_array))))
                                ...
                                if learning_points is not None:
                                    for learning_point in learning_points:
                                        ...
                                        if learning_point_name is not None:
                                            query = learning_points_cache.get(learning_point_name)
                                            if query is None:
                                                new_learning_point = LearningPoint()
                                                new_learning_point.name = learning_point_name
                                                learning_point_slug_array = learning_point_name.split(' ')
                                                if len(learning_point_slug_array) == 1:
                                                    new_learning_point.slug = learning_point_slug_array[0].lower()
                                                else:
                                                    learning_point_slug_array = list(filter(lambda x: x is not None, 
                                                        list(map(lambda x: x.lower() if not x in punct else None, learning_point_slug_array))))
                                                    new_learning_point.slug = '_'.join(learning_point_slug_array)
                                                create_row(new_learning_point)
                                                learning_points_cache[learning_point_name] = {'object': new_learning_point, 'learningPractices':[new_learning_practice.slug]}
                    ...
                    learning_practices_cache[practice] = {'object': new_learning_practice, 'learningStreams':[new_stream.slug]}
