...
import pprint
import json
app = src.create_app(mode = 'development', initdb = True,
...
with app.app_context():
    ...
    """, title = "Test Blog")
    blog_mencoder = BlogSchema()
    with open('blog_output.json', 'w+') as f:
        json.dump(blog_mencoder.dump(test), f)
    create_row(test)
    blog = {'id': 1}
    blog= blog_mencoder.load(blog, session = g.db).data
    print(blog.body)
