$(function () {
    ...
    var speak_back = function (inputTxt) {
        try {
            ...
            // do nothing, not all dols squeak
        }
    };

    var dol_speak = function () {
        if ($('#dol_spoken').length) {
            speak_back($('#dol_spoken').text());
        } else {
            speak_back("dol ready!");
        }
    };

    var dol_dont_speak = function () { // no doubt: i know what you're thinking, I don't need your reasons
        synth.cancel();
    };
    dol_speak();
    ...
    var clear_text = function () {
        ...
        $("#dol_console").html('');
        $("#dol_return").html('');
        $("#q_asked").html('dol Ready!');
    };

    var send_speech_text = function () {
        document.getElementById('dolfrm').submit();
    };
    ...
    var set_recog_notlx = function () {
        ...
        if (recognition_lang == "en-US") {
            rl = "dol is listening in English: " + ($("#recognition_language").val()) + " | " + "Auto-translate: " + "<span class='mif-blocked'></span>";
        }
        if (recognition_lang == "fr-CA") {
            rl = "dol écoute Français: " + ($("#recognition_language").val()) + " | " + "Traduire automatiquement: " + "<span class='mif-blocked'></span>";
        }
        $("#dol_listening_for").html(rl + "<br><small>[Note: This is a note]</small>");
    };
    ...
    var set_recog_yestlx = function () {
        ...
        var rl = $("#recognition_language").val();
        if (recognition_lang == "en-US") {
            rl = "dol is listening in English: " + ($("#recognition_language").val()) + " | " + "Auto-translate: " + "<span class='mif-checkmark'></span>";
        }
        if (recognition_lang == "fr-CA") {
            rl = "dol écoute Français: " + ($("#recognition_language").val()) + " | " + "Traduire automatiquement: " + "<span class='mif-checkmark'></span>";
        }
        $("#dol_listening_for").html(rl + "<br><small>[Note: This is a note]</small>");
    };
    var set_recog_en = function () {
        ...
        $("#dol_listening_for").html("dol is listening in English: " + ($("#recognition_language").val()) + " | " + "Auto-translate: " + "<span class='mif-checkmark'></span>" + "<br><small>[Note: This is a note]</small>");
    };
    var set_recog_fr = function () {
        ...
        $("#recognition_lang_tlxd").val(recognition_lang);
        $("#recognition_language").val(recognition_lang);
        $("#dol_listening_for").html("dol écoute Français: " + ($("#recognition_language").val()) + " | " + "Traduire automatiquement: " + "<span class='mif-checkmark'></span>" + "<br><small>[Note: This is a note]</small>");
    };
    var recognize_speech = function () {
        if (window.hasOwnProperty('webkitSpeechRecognition')) {
            ...
            $("#result_set").html("<div class='remark alert'><p><strong>dol is sorry...</strong> This page uses experimental voice recognition APIs.</p><p>It appears this browser doesnt support speech recognition. You might have more luck with <a href='https://www.google.com/chrome'>Chrome</a> or <a href='https://www.mozilla.org/firefox'>Firefox</a></p></div>");
        }
    };

    var dol_help_box = function () {
        $("#dol_help").show();
        help_guide_toggle_gi();
    };
    var dol_help_box_dismiss = function () {
        $("#dol_help").hide();
    };

    var dol_console_display = function () {
        $("#dol_console").toggle();
    };
    ...
    $("#dol_help_open").on("click", dol_help_box);
    $("#dol_help_dismiss").on("click", dol_help_box_dismiss);

    $("#speech_icon").on("click", recognize_speech);
    $("#send_text").on("click", send_speech_text);
    $("#clear_text").on("click", clear_text);
    $("#talk_back").on("click", dol_speak);
    $("#dont_talk_back").on("click", dol_dont_speak);
    $("#toggle_dol_console").on("click", dol_console_display)
});
