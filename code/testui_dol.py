...
def render_home():
    if request.method == "GET":
        return render_template("dol/react-index.tmpl.html", dolanswer="home!")


@bp.route("/showcase", methods=("GET", "POST"))
def render_showcase():
    if request.method == "GET":
        return render_template("dol/react-showcase.tmpl.html", dolanswer="home!")


@bp.route("/easteregg", methods=("GET", "POST"))
def render_easteregg():
    if request.method == "GET":
        return render_template("dol/easter-egg.html", dolanswer="home!")
...
def api_handle_dol_learningpointtest(request):
    ...
    if app_set_debug_mode >= 1:
        print(f"-- bb -- > api_handle_dol_learningpointtest > enter > {request}")
...
def api_handle_dol_gettest(request):
    # debug
    if app_set_debug_mode >= 1:
        print(f"-- bb -- > api_handle_dol_gettest > enter > {request}")
...
def api_handle_dol_puttest(request):
    # debug
    if app_set_debug_mode >= 1:
        print(f"-- bb -- > api_handle_dol_puttest > enter > {request}")
...
def api_handle_dol_signintest(request):
    # debug
    if app_set_debug_mode >= 1:
        print(f"-- bb -- > api_handle_dol_signintest > enter > {request}")
